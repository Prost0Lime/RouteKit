...
В тестах RouteKit не проявил признаки обычного Android VPN-клиента:

- `TRANSPORT_VPN`: не обнаружен.
- `VpnService`: не активен для RouteKit.
- TUN/TAP/WireGuard/PPP-like интерфейсы: не обнаружены.
- Default route остаётся через обычный интерфейс, например `wlan0`.
- `NET_CAPABILITY_NOT_VPN`: присутствует.
- Системный HTTP/SOCKS proxy через Android `ProxyInfo`: не настроен.
- Открытые localhost proxy после закрытия сторонних приложений: не обнаружены.
- IP-чекеры видят только внешний IP выхода, а не адрес конечного VLESS-сервера.

### Где Могут Быть Косвенные Сигналы

RouteKit не делает устройство “невидимым” для любых сетевых эвристик. Некоторые проверки всё ещё могут показывать `needs review`, если анализируют не Android VPN API, а поведение сети:

- ICMP/ping до заблокированных или нестандартно доступных целей может выглядеть необычно.
- RTT/latency может отличаться от ожидаемого маршрута.
- Сайт или приложение может сравнивать DNS, IP выхода, CDN-регион, TLS/HTTP-поведение или доступность конкретных ресурсов.
- Если на устройстве работает другое приложение с localhost proxy, проверка может увидеть его. Например, в одном тесте `127.0.0.1:41733` принадлежал `com.whatsapp`, а не RouteKit.

### Итог Проверки

В обновлённом тесте `rknhardering` с включённым RouteKit:

- прямых VPN-признаков не найдено;
- localhost proxy не найден;
- bypass не подтверждён;
- внешний IP совпал у RU и non-RU IP-чекеров;
- итоговый `NEEDS_REVIEW` остался только из-за ICMP/RTT-эвристики по `instagram.com`.

То есть RouteKit не определяется как Android VPN/TUN-клиент, но отдельные приложения всё равно могут строить косвенные выводы по сетевому поведению.
